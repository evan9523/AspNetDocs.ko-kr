...
**그림 2**: `NORTHWND.MDF` 폴더에서 데이터베이스 연결 `App_Data` ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image2.png))
...
[Objectdatasource를 사용 하 여 데이터 캐싱](caching-data-with-the-objectdatasource-cs.md) 자습서에서는 objectdatasource의 선언적 캐싱 기능을 탐색 했습니다. 속성을로 설정 하 `EnableCaching` `true` 고 속성을 `CacheDuration` 일정 시간 간격으로 설정 하면 ObjectDataSource가 지정 된 간격 동안 기본 개체에서 반환 된 데이터를 자동으로 캐시 합니다. ObjectDataSource는 하나 이상의 SQL 캐시 종속성을 사용할 수도 있습니다.
...
[![ProductsDataSourceDeclarative 라는 새 ObjectDataSource를 만듭니다.](using-sql-cache-dependencies-cs/_static/image5.gif)](using-sql-cache-dependencies-cs/_static/image3.png)

**그림 5**: 라는 새 ObjectDataSource 만들기 `ProductsDataSourceDeclarative` ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image4.png))
...
[![세 개의 입력 매개 변수를 사용 하 여 UpdateProduct 오버 로드 사용](using-sql-cache-dependencies-cs/_static/image6.gif)](using-sql-cache-dependencies-cs/_static/image5.png)

**그림 6**: 3 개의 입력 매개 변수를 사용 하 여 Updateproduct 오버 로드 사용 ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image6.png))

[![삽입 및 삭제 탭의 드롭다운 목록을 (없음)으로 설정 합니다.](using-sql-cache-dependencies-cs/_static/image7.gif)](using-sql-cache-dependencies-cs/_static/image7.png)
...
[![ObjectDataSource s 선택 이벤트는 GridView가 페이징, 편집 또는 정렬 될 때마다 발생 합니다.](using-sql-cache-dependencies-cs/_static/image8.gif)](using-sql-cache-dependencies-cs/_static/image9.png)
...
**그림 9**: 및에서 SQL 캐시 종속성을 사용 하 여 캐싱을 지원 하도록 ObjectDataSource 구성 `Products` `Categories` ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image12.png))
...
[![Products 테이블을 수정 하 여 캐시 된 제품 데이터 임의로](using-sql-cache-dependencies-cs/_static/image10.gif)](using-sql-cache-dependencies-cs/_static/image13.png)
...
[아키텍처의 데이터 캐싱](caching-data-in-the-architecture-cs.md) 자습서에서는 캐싱을 ObjectDataSource와 긴밀 하 게 결합 하는 것이 아니라 아키텍처에서 별도의 캐싱 계층을 사용 하는 이점을 살펴보았습니다. 이 자습서에서는 `ProductsCL` 데이터 캐시를 프로그래밍 방식으로 사용 하는 방법을 보여 주는 클래스를 만들었습니다. 캐싱 계층에서 SQL 캐시 종속성을 활용 하려면 클래스를 사용 `SqlCacheDependency` 합니다.
...
[![제품 Scl 클래스를 사용 하도록 ObjectDataSource 구성](using-sql-cache-dependencies-cs/_static/image11.gif)](using-sql-cache-dependencies-cs/_static/image15.png)

**그림 11**: 클래스를 사용 하도록 ObjectDataSource 구성 `ProductsCL` ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image16.png))

[![SELECT Tab s 드롭다운 목록에서 GetProducts 메서드를 선택 합니다.](using-sql-cache-dependencies-cs/_static/image12.gif)](using-sql-cache-dependencies-cs/_static/image17.png)

**그림 12**: `GetProducts` 탭 선택 드롭다운 목록에서 메서드 선택 ([전체 크기 이미지를 보려면 클릭](using-sql-cache-dependencies-cs/_static/image18.png))

[![업데이트 탭 s 드롭다운 목록에서 UpdateProduct 메서드를 선택 합니다.](using-sql-cache-dependencies-cs/_static/image13.gif)](using-sql-cache-dependencies-cs/_static/image19.png)
...
> [!NOTE]
> 이러한 지연은에서 GridView를 통해 제품 중 하나를 편집할 때 나타날 가능성이 높습니다 `SqlCacheDependencies.aspx` . [아키텍처의 데이터 캐싱](caching-data-in-the-architecture-cs.md) 자습서에서는 `MasterCacheKeyArray` 클래스의 메서드를 통해 편집 중인 데이터가 `ProductsCL` 캐시에서 제거 되도록 캐시 종속성을 추가 했습니다 `UpdateProduct` . 그러나이 단계 앞부분에서 메서드를 수정할 때이 캐시 종속성을 대체 했으므로이 `AddCacheItem` 클래스는 `ProductsCL` 폴링 시스템이 테이블 변경 내용을 확인할 때까지 캐시 된 데이터를 계속 표시 합니다 `Products` . 7 단계에서 캐시 종속성을 이동 하는 방법을 살펴보겠습니다 `MasterCacheKeyArray` .
